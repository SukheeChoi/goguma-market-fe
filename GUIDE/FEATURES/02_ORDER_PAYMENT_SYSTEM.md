...
토이 프로젝트나 백엔드가 아직 준비되지 않은 경우:

```typescript
// src/services/mockOrderService.ts
...
개발/프로덕션 환경에 따라 서비스 전환:

```typescript
// src/services/orderService.ts
const isDevelopment = process.env.NODE_ENV === 'development'
const useMockService = process.env.NEXT_PUBLIC_USE_MOCK_API === 'true'

// 개발 환경이거나 Mock API 사용 설정이면 Mock Service 사용
export * from isDevelopment || useMockService 
  ? './mockOrderService' 
  : './realOrderService'
```
...
### 🎯 실제 구현 예시 - localStorage를 활용한 Mock Service

```typescript
// src/services/orderService.ts - 전체 구현
import type { Order, OrderStatus, PaymentStatus } from '@/types/order'
...
1. **localStorage 활용**
   - 브라우저 새로고침해도 주문 데이터 유지
   - 실제 데이터베이스처럼 동작
   - 개발 중 테스트하기 편리

2. **네트워크 시뮬레이션**
   - 2초 지연으로 실제 API 호출 체감
   - 로딩 상태 테스트 가능
   - 비동기 처리 학습
...
**CheckoutPage에서 "결제하기" 버튼을 누르면:**

```
1. handleSubmit() 실행
   ↓
2. createOrder(items) - 장바구니 아이템으로 주문 생성
   ↓
3. setShippingAddress() - 배송지 정보 설정
   ↓
4. setPaymentMethod() - 결제 방법 설정
   ↓
5. submitOrder() 실행
   ↓
6. orderService.createOrder() 호출
   ↓
7. 2초 대기 (네트워크 시뮬레이션)
   ↓
8. 주문 데이터 localStorage에 저장
   ↓
9. router.push(`/payment/${order.id}`) - 결제 페이지로 이동
```
...
```javascript
// Application → Local Storage → mock_orders
[
  {
...
```typescript
// src/services/orderService.ts - 실제 API 버전
import { api } from '@/lib/api'
